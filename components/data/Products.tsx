...
function Products() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const resImg = await storage.listFiles('645fb16220a2f25a97c3');
      const { files } = resImg;
      if (files) {
    ...
    <>
        ...
        <div className="flex flex-1 my-2  h-[200px] w-auto ">
            <div className="bg-[#402D49]  w-auto mx-auto gap-4 shadow-[#000000] my-auto shadow-lg rounded-[12px]   ">
                ...
                <div onClick={() => router.push(`/product`)} className='hover:bg-form cursor-pointer flex text-center justify-items-center gap-2 items-center bg-orange m-auto px-4 py-2 text-white rounded-md '>
